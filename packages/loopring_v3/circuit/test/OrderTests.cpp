...
TEST_CASE("Order", "[OrderGadget]")
{
    auto orderChecked = [&](const FieldT &_exchange, const Order &order, bool expectedSatisfied) {
        ...
        OrderGadget orderGadget(pb, constants, exchange, maxFeeBips, constants._1, constants._0, ".order");
    ...
    std::cout << "in Order test: tx:" << tx.witness.balanceUpdateFee_A.after.balance << std::endl;
    ...
    SECTION("Valid order")
    {
        std::cout << "in Order test: start valid order" << std::endl;
